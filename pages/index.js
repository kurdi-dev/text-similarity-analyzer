...
import {
  Container, VStack, StackDivider, Box, Center, Input, Flex, Button,
  Table,
  Thead,
  Tbody,
  Tr,
...
export default function Home() {
  ...
  const handleOnClickCompare = async () => {
    ...
    let cosineSimilarityRatio = Similarity.cosine.similarity(firstText, secondText);
    let JaccardIndexRatio = Similarity.jaccardIndex.similarity(firstText, secondText);
    let lcsRatio = Similarity.lcs.similarity(firstText, secondText);
    let levenshteinDistanceSimilarityRatio = Similarity.levenshtein.similarity(firstText, secondText);
    let diceCoefficientRatio = Similarity.diceCoefficient.similarity(firstText, secondText);
      ...
      <main>
        <Container maxW='4xl' >
          <VStack
            ...
            align='stretch'
          >
            <Box mt={25} w='100%' bg='yellow.200'>
              <Center p={5}>
                This is a simple tool to compare two texts using different text comparison algorithms, you can analyze the comparition ratios to find the best algorithm for your case.
              </Center>
            ...
            <Box w='100%'>
              <Flex justify='space-between' alignItems='center' gap={2}>
                ...
                <Box>
                  <Button isLoading={isComparing} onClick={handleOnClickCompare} loadingText='Comparing' colorScheme='teal' size='md'>Compare</Button>
                </Box>
              </Flex>
            </Box>
            <Box w='100%'>
              <TableContainer>
                <Table variant='simple' size='sm'>
                  ...
                  <Tbody>
                    {tableRows.map((row, index) => <RenderTableRow obj={row} key={index} />)}
                  </Tbody>
                </Table>
              </TableContainer>
            </Box>
